  ...
  return (
    <div className="min-h-screen bg-green-50 text-gray-800 flex flex-col items-center px-4 py-10">
      ...
      <section className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 w-full max-w-5xl">
        {[
          { title: 'Workouts', desc: 'Browse curated training routines', color: 'bg-blue-100 text-blue-600', path: '/workouts' },
          { title: 'Nutrition', desc: 'Explore healthy meals and diets', color: 'bg-green-100 text-green-600', path: '/nutrition' },
          { title: 'Products', desc: 'Shop supplements & gear', color: 'bg-yellow-100 text-yellow-600', path: '/products' },
          { title: 'Community', desc: 'Share your journey with others', color: 'bg-purple-100 text-purple-600', path: '/community' },
          { title: 'Profile', desc: 'Track progress & goals', color: 'bg-red-100 text-red-600', path: '/profile' },
        ].map((item) => (
          <div
            key={item.title}
